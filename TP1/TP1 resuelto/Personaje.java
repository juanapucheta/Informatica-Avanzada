...
public class Personaje {
    ...
    // objeto -> el elemento que tiene en sus manos. Puede tener las manos vacias (null)
    ...
    public void setBolsa(Bolsa bolsa) {
        if(bolsa.getPesoMaximo() <= PESO_MAXIMO_BOLSA && (this.bolsa == null || bolsa.getPesoMaximo() > this.bolsa.getPesoMaximo())) {
    ...
    public void tomarElemento (String nombre) {
        Elemento encontrado = bolsa.delElemento(nombre);
    ...
    public void prepararReceta (Receta receta) {
        caldero.setReceta(receta); //prepara la pocima de la receta en el caldero
        
        //busco en la bolsa los ingredientes necesarios
        for(String nombreIngrediente : receta.getIngredientes()){
            Elemento ingrediente = bolsa.delElemento(nombreIngrediente);
            
            if(ingrediente != null){
                caldero.addIngrediente(ingrediente);
                }
        }
        
        List<String> faltantes = caldero.getIngredientesFaltantes();
        if(faltantes.size() > 0){
